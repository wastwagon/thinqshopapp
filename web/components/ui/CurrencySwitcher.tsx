    ...
    return (
        <div ref={ref} className="relative">
            <button
                type="button"
                onClick={() => setOpen((o) => !o)}
                className="flex items-center gap-1 sm:gap-1.5 min-h-[44px] px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg sm:rounded-xl border border-gray-200 bg-white hover:border-brand/30 hover:bg-brand/5 transition-all text-sm font-semibold text-gray-700 hover:text-brand focus:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 min-w-[56px] sm:min-w-[72px] justify-between shrink-0"
                aria-expanded={open}
            ...
            >
                <span className="flex items-center gap-1 sm:gap-1.5">
                    ...
                    <span className="text-sm sm:text-base font-bold text-brand">{current.symbol}</span>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                        {CURRENCIES.map((c) => (
                            ...
                            >
                                ...
                                <span className="text-lg font-bold text-brand">{c.symbol}</span>
                                <span className="text-sm font-semibold">{c.label}</span>
                                {currency === c.code && (
                                    <span className="ml-auto text-xs font-bold uppercase tracking-wider text-brand">✓</span>
                                )}
                            </button>
                        ))}
                    </div>
