...
export default function DashboardWalletCard({ balance, hidden, onToggleHidden }: DashboardWalletCardProps) {
    ...
    return (
        <motion.div
            ...
            className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-950 via-[#02274f] to-blue-900 p-4 shadow-[0_8px_32px_-8px_rgba(2,39,79,0.45)] min-h-[88px] flex items-center"
        >
            ...
            <div className="relative flex items-center justify-between gap-3 w-full min-w-0">
                <div className="min-w-0 flex-1">
                    ...
                    <p className="text-white font-bold tracking-tight truncate">
                        {hidden ? (
                            <span className="text-xl">••••••</span>
                        ) : (
                            <>
                                <span className="text-sm font-semibold text-blue-100/90 mr-1">GHS</span>
                                <span className="text-2xl">{whole}</span>
                                <span className="text-lg text-blue-100/80">.{fraction ?? '00'}</span>
                            </>
                        )}
                    </p>
                </div>
                <Link
                    ...
                    className="shrink-0 w-11 h-11 rounded-xl bg-white text-blue-950 flex items-center justify-center shadow-lg hover:bg-blue-50 active:scale-95 transition-all"
