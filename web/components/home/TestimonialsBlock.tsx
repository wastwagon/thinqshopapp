...
export default function TestimonialsBlock({ testimonials }: { testimonials: Testimonial[] }) {
    ...
    return (
        <section className="py-8 sm:py-12 bg-white border-y border-gray-100" aria-label="Testimonials">
            <div className="max-w-6xl mx-auto px-4 sm:px-6">
                <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-1">What our customers say</h2>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    {list.slice(0, 6).map((t) => (
                        <blockquote
                            key={t.id}
                            className="p-4 sm:p-5 rounded-2xl bg-gray-50 border border-gray-100 min-h-[120px] flex flex-col"
                        >
                            <Quote className="w-8 h-8 text-blue-500/30 mb-2 flex-shrink-0" aria-hidden />
