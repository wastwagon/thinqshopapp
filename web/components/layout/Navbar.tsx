    ...
    return (
        ...
        >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 sm:h-16 w-full flex items-center gap-4 sm:gap-8">
                ...
                <div className="flex items-center gap-2 lg:hidden">
                    <button
                        ...
                        className="touch-target w-12 h-12 -ml-2 rounded-2xl flex items-center justify-center text-gray-600 hover:bg-gray-100 hover:text-blue-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 transition-colors"
            ...
            <div
                id="mobile-main-menu"
                ref={menuRef}
                className={`lg:hidden fixed inset-0 top-14 sm:top-16 z-40 bg-white/95 backdrop-blur-xl transform transition-transform duration-300 ease-out ${
            ...
            >
                <div className="px-6 py-8">
                    <button
                        ...
                        className="w-full mb-8 flex items-center gap-3 px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-gray-500 hover:bg-gray-100 hover:text-gray-900 transition-colors"
                    >
                    ...
                    <ul className="space-y-1" role="menu">
                        <li role="none">
                            <Link
                                ...
                                role="menuitem"
                                onClick={closeMenu}
                                className="block py-4 px-5 rounded-2xl text-[13px] font-bold uppercase tracking-wider transition-colors hover:bg-gray-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-inset text-gray-700"
                            >
                                Shop
                            </Link>
                            <ul className="ml-4 mt-2 space-y-0.5 pb-4 border-l-2 border-gray-200 pl-4">
                                <li>
                                    <Link href="/shop" onClick={closeMenu} className="block py-2 px-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900">All Products</Link>
                                </li>
                                {STATIC_CATEGORIES.map((cat) => {
                                    const slug = cat.slug ?? cat.name.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, '');
                                    return (
                                        <li key={cat.slug}>
                                            <Link href={`/shop/${slug}`} onClick={closeMenu} className="block py-2 px-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-900">{cat.name}</Link>
                                        </li>
                                    );
                                })}
                            </ul>
                        ...
                        {mainNavItems.filter((i) => !i.mega).map((item) => (
                            <li key={item.href} role="none">
                                <Link
                                    ...
                                    role="menuitem"
                                    onClick={closeMenu}
                                    className={`block py-4 px-5 rounded-2xl text-[13px] font-bold uppercase tracking-wider transition-colors hover:bg-gray-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-inset ${
